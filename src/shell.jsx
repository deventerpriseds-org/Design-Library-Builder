...
export function Shell({ children }) {
  ...
  // Close drawer on navigation or resize to desktop
    ...
    return (
      <>
        {NAV.map((n) => {
          ...
          return (
            <button key={n.path} onClick={() => { go(n.path); onNavigate?.() }}
        ...
        <div style={{ padding: '12px 12px 4px', borderTop: '1px solid var(--dlg-border)' }}>
          ...
          {NAV.slice(0, 4).map((n, i) => {
            ...
            const stepDone = i < NAV.findIndex(m => m.path === active)
            return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', overflow: 'hidden' }}>
      ...
      <div style={{ height: 'var(--dlg-topbar)', display: 'flex', alignItems: 'center', gap: 10,
        ...
        background: 'var(--dlg-surface)', flexShrink: 0, zIndex: 20 }}>
        {isMobile && (
          <button className="dlg-btn dlg-btn-ghost" onClick={() => setDrawerOpen(o => !o)}
            style={{ padding: '0 8px', fontSize: 18, lineHeight: 1 }} aria-label="Menu">
            {drawerOpen ? '✕' : '☰'}
          </button>
        )}
        <div onClick={() => go('/upload')} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, letterSpacing: -0.3 }}>
            Design<span style={{ color: 'var(--dlg-brand)' }}>·</span>
            {isMobile ? 'Gen' : 'Library Gen'}
          </span>
        </div>
        <div style={{ flex: 1 }} />
        ...
        {signedIn ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {!isMobile && <span style={{ fontSize: 13, color: 'var(--dlg-text-2)', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{state.user.email}</span>}
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {(!isMobile || drawerOpen) && (
          <nav style={{
            width: 'var(--dlg-sidebar)', flexShrink: 0,
            borderRight: '1px solid var(--dlg-border)',
            background: 'var(--dlg-surface)', padding: '12px 8px',
            display: 'flex', flexDirection: 'column', gap: 2,
            ...(isMobile ? {
              position: 'absolute', top: 0, left: 0, bottom: 0, zIndex: 16,
              boxShadow: 'var(--dlg-shadow-lg)',
            } : {}),
          }}>
            <NavItems onNavigate={isMobile ? () => setDrawerOpen(false) : undefined} />
          </nav>
        )}

        {/* Main content */}
        <main style={{ flex: 1, overflow: 'auto', background: 'var(--dlg-bg)', minWidth: 0 }}>
          {children}
        </main>
      </div>
      <DebugPanel />
